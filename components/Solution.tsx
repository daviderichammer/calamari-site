        ...
        <div className="relative mb-20">
          ...
          <div className="terminal max-w-5xl mx-auto overflow-hidden rounded-2xl shadow-2xl glow-blue">
            ...
            <div className="terminal-header">
              ...
              <span className="ml-4 text-brand-text text-xs font-mono">dashboard.calamaritrading.ai</span>
            ...
            <div className="p-6 bg-[#0A0B14]">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                {[
                  ...
                  <div key={i} className="bg-[#0F1120] rounded-xl p-4 border border-[#1E2035]">
                    <div className="text-xs text-brand-text mb-1">{stat.label}</div>
                    <div className="text-lg font-bold text-brand-heading">{stat.value}</div>
              ...
              <div className="space-y-3">
                <div className="text-xs text-brand-text uppercase tracking-wider mb-3 font-semibold">
                ...
                {[
                  ...
                  <div
                    key={i}
                    className="flex items-center justify-between bg-[#0F1120] rounded-xl px-4 py-3 border border-[#1E2035] hover:border-brand-blue/30 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
                      <div>
                        <div className="text-sm font-semibold text-brand-heading">{account.name}</div>
                        <div className="text-xs text-brand-text">{account.firm}</div>
                      </div>
                    </div>
                    <div className="hidden sm:flex items-center gap-6 text-sm">
                      <div className="text-brand-heading font-mono">{account.balance}</div>
                      <div className="text-green-400 font-mono font-semibold">{account.pl}</div>
                      <div className="text-xs text-brand-text">{account.positions} positions</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
          {[
            ...
            <div key={i} className="relative text-center">
              {i < 2 && (
                <div className="hidden md:block absolute top-8 left-full w-full h-px bg-gradient-to-r from-brand-blue/50 to-transparent -translate-x-4 z-0" />
              )}
              <div className="relative z-10">
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl gradient-border mb-4">
