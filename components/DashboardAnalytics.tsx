...
const tabs = [
  ...
  { id: "risk", label: "Risk Monitoring", icon: "🛡️" },
    ...
    <section id="dashboard" className="relative py-24 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-brand-text max-w-3xl mx-auto mb-8">
          ...
          <div className="inline-flex flex-wrap justify-center gap-3 mb-4">
            {["fxmonitor", "fxmon", "MyFxBook", "FX Blue Online"].map((tool) => (
              <div key={tool} className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-red-500/10 border border-red-500/20 text-red-400 text-sm">
                <span className="line-through opacity-60">{tool}</span>
                <span className="text-xs">✕</span>
              </div>
            ))}
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-green-500/10 border border-green-500/20 text-green-400 text-sm font-semibold">
              <span>✓</span>
        ...
        <div className="mb-16 rounded-2xl gradient-border p-8 max-w-4xl mx-auto">
          <div className="flex flex-col md:flex-row gap-6 items-center">
            <div className="flex-shrink-0 text-center">
              <div className="text-6xl font-black gradient-text mb-1">2.84</div>
              <div className="text-brand-cyan font-bold text-lg">Calmar Ratio</div>
              <div className="text-xs text-brand-text mt-1">Annualized Return ÷ Max Drawdown</div>
            </div>
            <div className="flex-1 md:border-l md:border-brand-border md:pl-8">
            ...
            <div className="space-y-8">
              ...
              <div className="terminal rounded-2xl overflow-hidden shadow-2xl glow-blue">
                <div className="terminal-header">
                  ...
                  <span className="ml-4 text-brand-text text-xs font-mono">Performance Analytics — All Accounts</span>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {performanceMetrics.map((m, i) => (
                  ...
                  >
                    {m.highlight && (
                      <div className="text-xs text-brand-cyan font-semibold mb-1 uppercase tracking-wider">★ Headline Metric</div>
                    )}
                    <div className="text-xs text-brand-text mb-1">{m.label}</div>
                    <div className={`text-2xl font-black font-mono ${m.color}`}>{m.value}</div>
                    <div className="text-xs text-brand-text mt-1 leading-tight">{m.desc}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={i} className="flex gap-4 p-4 rounded-xl bg-brand-card border border-brand-border hover:border-brand-blue/30 transition-colors">
            ...
            <div className="space-y-8">
              <div className="terminal rounded-2xl overflow-hidden shadow-2xl glow-blue">
                <div className="terminal-header">
                  ...
                  <div className="terminal-dot bg-yellow-500" />
                  <div className="terminal-dot bg-green-500" />
                  <span className="ml-4 text-brand-text text-xs font-mono">Trade Analytics — May 2025</span>
                  <div className="ml-auto flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
                    <span className="text-green-400 text-xs font-mono">Live</span>
                  </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                {tradeMetrics.map((m, i) => (
                  <div key={i} className="rounded-xl p-4 bg-brand-card border border-brand-border text-center hover:border-brand-blue/30 transition-colors">
                    <div className="text-xs text-brand-text mb-1">{m.label}</div>
            ...
            <div className="space-y-8">
              <div className="terminal rounded-2xl overflow-hidden shadow-2xl glow-blue">
                <div className="terminal-header">
                  ...
                  <div className="terminal-dot bg-yellow-500" />
                  <div className="terminal-dot bg-green-500" />
                  <span className="ml-4 text-brand-text text-xs font-mono">Risk & Compliance Monitor — Real-time</span>
                  <div className="ml-auto flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
                    <span className="text-green-400 text-xs font-mono">All rules passing</span>
                  </div>
                </div>
                <Image
              ...
              <div className="rounded-2xl bg-brand-card border border-brand-border p-6">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="font-bold text-brand-heading">Live Prop Firm Compliance</h3>
                    ...
                    <div key={i} className="space-y-1.5">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className={`font-mono font-semibold ${item.status === "warn" ? "text-amber-400" : "text-green-400"}`}>
                          {item.current}{item.limit ? ` / ${item.limit} limit` : ""}
                        </span>
                      </div>
                ...
                <div className="mt-6 p-3 rounded-xl bg-green-500/10 border border-green-500/20 text-center">
                  <span className="text-green-400 text-sm font-semibold">✓ All prop firm rules: COMPLIANT — 3.2% of 5% daily limit used</span>
                </div>
              </div>
            ...
            <div className="space-y-8">
              <div className="terminal rounded-2xl overflow-hidden shadow-2xl glow-blue">
                <div className="terminal-header">
                  ...
                  <div className="terminal-dot bg-yellow-500" />
                  <div className="terminal-dot bg-green-500" />
                  <span className="ml-4 text-brand-text text-xs font-mono">All Accounts Overview — 7/7 Active</span>
                  <div className="ml-auto flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {[
                  ...
                  <div key={i} className="rounded-xl p-4 bg-brand-card border border-brand-border text-center">
              ...
              <div className="space-y-3">
                ...
                {accounts.map((acc, i) => (
                  <div
                    key={i}
                    className="flex flex-col sm:flex-row sm:items-center justify-between bg-brand-card rounded-xl px-4 py-3 border border-brand-border hover:border-brand-blue/30 transition-colors gap-3"
                  >
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-sm">
                      <div className="text-center">
                        ...
                        <div className="font-mono font-semibold text-brand-heading">{acc.balance}</div>
                      </div>
                      <div className="text-center">
                        <div className="text-xs text-brand-text">Equity</div>
                        <div className="font-mono font-semibold text-brand-heading">{acc.equity}</div>
                      </div>
                      <div className="text-center">
                        <div className="text-xs text-brand-text">Floating P&L</div>
                        <div className="font-mono font-semibold text-green-400">{acc.pl}</div>
                      </div>
                      <div className="text-center">
                        <div className="text-xs text-brand-text">Positions</div>
                        <div className="font-mono font-semibold text-brand-cyan">{acc.positions}</div>
                      </div>
        ...
        <div className="mb-16 rounded-2xl bg-brand-card border border-brand-border overflow-hidden">
          <div className="p-6 border-b border-brand-border">
            <h3 className="text-xl font-bold text-brand-heading text-center">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-brand-border">
                  <th className="text-left px-6 py-4 text-brand-text font-medium">Tool you can cancel</th>
                  <th className="text-center px-4 py-4 text-brand-text font-medium">Monthly cost</th>
                  <th className="text-left px-4 py-4 text-brand-text font-medium">What it does</th>
                  <th className="text-center px-4 py-4 text-brand-cyan font-medium">Calamari</th>
                </tr>
              </thead>
              <tbody>
                {replacements.map((row, i) => (
                  <tr key={i} className="border-b border-brand-border/50 hover:bg-brand-blue/5 transition-colors">
                    ...
                    <td className="px-4 py-4 text-brand-text">{row.feature}</td>
                    <td className="px-4 py-4 text-center">
                ...
                <tr className="bg-green-500/5">
                  <td className="px-6 py-4 font-bold text-brand-cyan">Calamari Dashboard</td>
                  <td className="px-4 py-4 text-center">
                    <span className="text-green-400 font-bold font-mono">$0 extra</span>
                  </td>
                  <td className="px-4 py-4 text-brand-text">All of the above + more</td>
                  <td className="px-4 py-4 text-center">
          ...
          <div className="p-6 bg-brand-blue/5 border-t border-brand-border">
        ...
        <div className="mb-16">
          <div className="text-center mb-10">
            <h3 className="text-2xl sm:text-3xl font-black text-brand-heading mb-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              <div
                key={i}
                className={`group relative rounded-2xl p-6 bg-brand-card border border-brand-border ${item.border} transition-all duration-300 hover:-translate-y-1`}
              >
        ...
        {/* Bottom CTA */}
        <div className="rounded-2xl gradient-border p-10 text-center max-w-3xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              href="/checkout"
              className="btn-primary inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl text-base font-bold text-white"
            >
            ...
            <a
              ...
              className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl text-base font-semibold text-brand-cyan border border-brand-cyan/30 hover:bg-brand-cyan/10 transition-all duration-200"
            >
              View Live Demo
